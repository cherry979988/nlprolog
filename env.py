...
class BaseEnv:
    def __init__(self, data_dir, shuffle=True, evaluate=False, rules='random'):
        ...
        with open(os.path.join(f'{utils.BASE_PATH}/data', data_dir, f'{split_type}.json')) as f:
            ...
            for d in self.data:
                ...
                ## adding supports generated by RE models
                re_new_supports = []
                if 're_supports' in d:
                    for support in d['re_supports']:
                        # last field is confidence score.
                        re_new_supports.append([s.lower() for s in support[:-1]] + [float(support[-1])])
                d['re_supports'] = re_new_supports
            ...
            if len(no_support_data):
                print(
                    f"WARNING: Removed {len(no_support_data)} problems because of they did not have any supports. This is problematic for the evaluation!")
    ...
    def _get_problem(self, i):
        ...
        self.current_program = prolog.Program(self.current_problem['supports'], self.current_problem['re_supports'], self.rules, candidate_triples)
    ...
    def _run_query(self, candidate_idx=None):
        '''
        Query the current program. save facts, rules, sims to tmp files
        '''

        cut = prolog.LAMBDA_CUT_PRE  # 0.5
        ...
        query_str = '|'.join(
            self.current_program.triple_to_prolog(c, symbol=True, invert=False) + '.' for c in candidates)
        ...
        # print(prolog_prog, sims)
        result = prolog.query(prolog_prog, sims, query_str, entity_tnorm=self.entity_tnorm,
                              predicate_tnorm=self.predicate_tnorm, min_depth=self.min_depth,
                              min_bs_size=self.min_bs_size, lambda_cut=self.lambda_cut, max_depth=self.max_depth)
